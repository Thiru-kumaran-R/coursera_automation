...
public class LearningPage {
    ...
    public List<String> extractAllLevels(){
        ...
        for (WebElement item : levelItems) {
            try {
                String levelName = item.findElement(
                        By.cssSelector("label div span:not(.css-ebbjlp)")
                ).getText().trim();

                // Strip trailing parenthesis group from the level name, if present
                // e.g., "Beginner(11,133)" -> "Beginner"
                levelName = levelName.replaceAll("\\s*\\(.*\\)\\s*$", "");

                String count = "";
                try {
                    count = item.findElement(By.cssSelector("label div span.css-ebbjlp"))
                            .getText().trim();
                } catch (NoSuchElementException ignored) { }

                if (!count.isEmpty() && !count.startsWith("(")) {
                    count = "(" + count + ")";
                }

                if (!levelName.isEmpty()) {
                    String formatted = levelName + " --> " + (count.isEmpty() ? "" : count);
                    levels.add(formatted.trim());
                }
